...
await wolf(structured_model=DiscussionModelCN) 2. 结构化输出

class VoteModelCN(BaseModel):
vote: str = Field(description="投票目标玩家姓名")
reason: str = Field(description="投票理由")
confidence: int = Field(ge=1, le=10, description="信心程度") 3. 并发管道
...
def get_role_prompt(role: str, character: str) -> str: # 自定义角色提示词逻辑
pass
🐛 常见问题
Q: 游戏无法启动？
A: 检查以下几点：
